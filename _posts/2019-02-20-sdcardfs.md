...
![这里写图片描述](./SDcardFS文件系统浅析（一）%20-%20sdcarfs历史%20-%20pen_cil的博客%20-%20CSDN博客_files/20180401180743655)\
从用户空间到列表文件（ls -l / tmp /
fuse）的请求被内核通过VFS重定向到FUSE。FUSE然后执行注册处理程序（./hello）并将请求传递给它（ls
-l / tmp /
fuse）。处理程序返回一个响应给FUSE，然后它被重定向到最初发出请求的用户空间程序。

由上图可以看出Fuse需要进行多次的用户态与内核态交互，这样就会造成切换开销

-   实例分析

    摘自国外一个博客[Why does FUSE on Android
    suck](http://fixbugfix.blogspot.com/2015/11/why-does-fuse-on-android-suck.html)
...
-   性能
    ...
    **EXT4**
    ...
    ![这里写图片描述](./SDcardFS文件系统浅析（一）%20-%20sdcarfs历史%20-%20pen_cil的博客%20-%20CSDN博客_files/20180401180337532)

    ![这里写图片描述](./SDcardFS文件系统浅析（一）%20-%20sdcarfs历史%20-%20pen_cil的博客%20-%20CSDN博客_files/20180401180355605)
...
![这里写图片描述](./SDcardFS文件系统浅析（一）%20-%20sdcarfs历史%20-%20pen_cil的博客%20-%20CSDN博客_files/20180401180434129)
...
![这里写图片描述](./SDcardFS文件系统浅析（一）%20-%20sdcarfs历史%20-%20pen_cil的博客%20-%20CSDN博客_files/2018040118050298)

**WrapFS 与EXT4性能对比图**

![这里写图片描述](./SDcardFS文件系统浅析（一）%20-%20sdcarfs历史%20-%20pen_cil的博客%20-%20CSDN博客_files/2018040118054784)

![这里写图片描述](./SDcardFS文件系统浅析（一）%20-%20sdcarfs历史%20-%20pen_cil的博客%20-%20CSDN博客_files/20180401180614500)\
